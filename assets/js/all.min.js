...
$(function() {
    ...
    window.setInterval(function() {
        var e = n.substring(0, d);
        d > n.length && (d = o = 0), 3 < o && (d++, o = 0), document.title = e + t[o], o++
    }, 150);
    var a = ["B4ZYH1GH"],
        r = ["BLEACH", "NARUTO", "SAO", "BARUTO", "DXD"],
        u = ["ssg", "ak47", "deagle", "g3sg1"],
        i = $(".kill-feed"),
        l = $(".kill-feed > div").hide();

    function e(e) {
        e || (e = window.event), e.stopPropagation ? e.stopPropagation() : e.cancelBubble = !0, e.preventDefault ? e.preventDefault() : e.returnValue = !1
    }
    window.setInterval(function() {
        var e = l.clone();
        e.find(".weapons img:first-child").attr("src", "images/" + u[Math.floor(Math.random() * u.length)] + ".png"), e.find(".t").text(a[Math.floor(Math.random() * a.length)]), e.find(".ct").text(r[Math.floor(Math.random() * r.length)]), i.append(e.show().delay(1e3).fadeOut(1e3, function() {
            $(this).remove()
        }))
    }, 250), document.onkeydown = function(e) {
        return e.ctrlKey && (67 === e.keyCode || 86 === e.keyCode || 85 === e.keyCode || e.keyCode), !1
    }, document.onmousedown = function() {
        2 != event.button && 3 != event.button || (oncontextmenu = "return false")
    }, document.oncontextmenu = new Function("return false;"), document.body.addEventListener && document.body.addEventListener("DOMMouseScroll", e, !1), document.body.onmousewheel = e
});
/* ===== unset ====== */
function noselect() {
    return !1
...
function getWebGLContext(e) {
    var r = {
        ...
        i = !!t;
    i || (t = e.getContext("webgl", r) || e.getContext("experimental-webgl", r));
    var a = t.getExtension("OES_texture_half_float"),
        o = t.getExtension("OES_texture_half_float_linear");
    return i && (t.getExtension("EXT_color_buffer_float"), o = t.getExtension("OES_texture_float_linear")), t.clearColor(0, 0, 0, 1), {
        gl: t,
        ext: {
            internalFormat: i ? t.RGBA16F : t.RGBA,
            internalFormatRG: i ? t.RG16F : t.RGBA,
            formatRG: i ? t.RG : t.RGBA,
            texType: i ? t.HALF_FLOAT : a.HALF_FLOAT_OES
        },
...
var GLProgram = function() {
    function e(r, t) {
        ...
        if (this.uniforms = {}, this.program = gl.createProgram(), gl.attachShader(this.program, r), gl.attachShader(this.program, t), gl.linkProgram(this.program), !gl.getProgramParameter(this.program, gl.LINK_STATUS)) throw gl.getProgramInfoLog(this.program);
        for (var i = gl.getProgramParameter(this.program, gl.ACTIVE_UNIFORMS), a = 0; a < i; a++) {
            var o = gl.getActiveUniform(this.program, a).name;
...
function initFramebuffers() {
    ...
    var e = ext.internalFormat,
        r = ext.internalFormatRG,
        t = ext.formatRG,
        i = ext.texType;
    density = createDoubleFBO(0, textureWidth, textureHeight, e, gl.RGBA, i, support_linear_float ? gl.LINEAR : gl.NEAREST), velocity = createDoubleFBO(2, textureWidth, textureHeight, r, t, i, support_linear_float ? gl.LINEAR : gl.NEAREST), divergence = createFBO(4, textureWidth, textureHeight, r, t, i, gl.NEAREST), curl = createFBO(5, textureWidth, textureHeight, r, t, i, gl.NEAREST), pressure = createDoubleFBO(6, textureWidth, textureHeight, r, t, i, gl.NEAREST)
}

function createFBO(e, r, t, i, a, o, n) {
    gl.activeTexture(gl.TEXTURE0 + e);
    var l = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, l), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, n), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, n), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE), gl.texImage2D(gl.TEXTURE_2D, 0, i, r, t, 0, a, o, null);
...
function createDoubleFBO(e, r, t, i, a, o, n) {
    var l = createFBO(e, r, t, i, a, o, n),
        u = createFBO(e + 1, r, t, i, a, o, n);
    return {get first() {
            return l
        },
        get second() {
...
function update() {
    resizeCanvas();
    var e = Math.min((Date.now() - lastTime) / 1e3, .016);
    if (lastTime = Date.now(), gl.viewport(0, 0, textureWidth, textureHeight), splatStack.length > 0)
        for (var r = 0; r < splatStack.pop(); r++) {
            var t = [10 * Math.random(), 10 * Math.random(), 10 * Math.random()];
            splat(canvas.width * Math.random(), canvas.height * Math.random(), 1e3 * (Math.random() - .5), 1e3 * (Math.random() - .5), t)
        }
    ...
    for (var i = 0, a = pointers.length; i < a; i++) {
        var o = pointers[i];
        o.moved && (splat(o.x, o.y, o.dx, o.dy, o.color), o.moved = !1)
    }
...
function renderTime() {
    var e = new Date,
        ...
        i = e.getMinutes(),
        a = e.getSeconds();
    setTimeout("renderTime()", 1e3), 0 == t ? t = 12 : t > 12 && (t -= 12, r = "PM"), t < 10 && (t = "0" + t), i < 10 && (i = "0" + i), a < 10 && (a = "0" + a);
    var o = document.getElementById("clockDisplay");
    e = document.querySelectorAll('.hover-ch');
    e[0].textContent = t;
    e[1].textContent = i;
    e[2].textContent = a;
    e[3].textContent = r;
...
document.addEventListener("mousemove", function(e) {
    ++count > 25 && (colorArr = [Math.random() + .2, Math.random() + .2, Math.random() + .2], count = 0), pointers[0].down = !0, pointers[0].color = colorArr, pointers[0].moved = pointers[0].down, pointers[0].dx = 10 * (e.clientX - pointers[0].x), pointers[0].dy = 10 * (e.clientY - pointers[0].y), pointers[0].x = e.clientX, pointers[0].y = e.clientY
}), document.addEventListener("touchmove", function(e) {
    e.preventDefault();
    var r = e.targetTouches;
    ++count > 25 && (colorArr = [Math.random() + .2, Math.random() + .2, Math.random() + .2], count = 0);
    for (var t = 0, i = r.length; t < i; t++) {
        t >= pointers.length && pointers.push(new pointerPrototype), pointers[t].id = r[t].identifier, pointers[t].down = !0, pointers[t].x = r[t].pageX, pointers[t].y = r[t].pageY, pointers[t].color = colorArr;
        var a = pointers[t];
        a.moved = a.down, a.dx = 10 * (r[t].pageX - a.x), a.dy = 10 * (r[t].pageY - a.y), a.x = r[t].pageX, a.y = r[t].pageY
    }
}, !1), renderTime();
/* ====== xxxx ======= */
...
function pVid() {
    const elem = document.querySelector('#pp-btn');
    if (audio.paused) {
...
let dt = document.querySelector('#doc-title');
let tc = dt.textContent;
dt.innerHTML = '';
tc.split('').forEach(c => {
  let ch = document.createElement('span');
  ch.classList.add('hover-effect');
  ch.textContent = c;
  dt.appendChild(ch);
});
